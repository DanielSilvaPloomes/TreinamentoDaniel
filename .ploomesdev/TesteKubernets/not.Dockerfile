...
FROM mcr.microsoft.com/dotnet/aspnet:8.0 as serve-stage
WORKDIR /app
COPY startup.sh .
...
<@ DEBUG_ONLY_BEGIN @>
    ...
    ENTRYPOINT service ssh restart && bash startup.sh && dotnet "TesteKubernets.dll"
<@ DEBUG_ONLY_END @>

<@ RELEASE_ONLY_BEGIN @>
    ENTRYPOINT bash startup.sh && dotnet "TesteKubernets.dll"
<@ RELEASE_ONLY_END @>
